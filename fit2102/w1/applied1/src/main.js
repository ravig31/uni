 ...
 */
const removeOnes = arr => arr.filter(x => (x === 1 ? false : true));
 ...
 */
const sumArray = arr => arr.reduce((accumulator, x) => accumulator + x, 0);
...
const filterEvenNumbers = array =>
    array.filter(x => (x % 2 === 0 ? true : false));
...
const tripleAndFilterOdds = array =>
    array.map(x => x * 3).filter(x => (x % 2 !== 0 ? true : false));

const countOddNumbers = array =>
    array
        .filter(x => (x % 2 !== 0 ? true : false))
        .reduce((count, x) => count + 1, 0);
...
const projectEulerProblem1Again = n =>
    range(1000).reduce(
        (sum, x) => (isDivisibleByThreeOrFive(x) ? sum + x : sum),
        0,
    );
