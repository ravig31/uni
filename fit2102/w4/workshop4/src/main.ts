...
import { fromEvent, interval, Subscription, Observable, merge, scan } from 'rxjs';
import { map, filter, reduce } from 'rxjs/operators';
import { Key, Event, Action, State, Constants } from './types'
import { Tick, Rotate, Thrust, Shoot, reduceState, initialState, UserTeleport, RandomTeleport } from './state';
import { IMPLEMENT_THIS, RNG } from './util';
...
function asteroids() {
	const
		...
		teleport$ = key$('keydown', 'Enter').pipe(map(_ => new UserTeleport())),
		shoot$ = key$('keydown', 'Space').pipe(map(_ => new Shoot())),
		randomTeleport$ = interval(Constants.RandomTeleportInterval)
			.pipe(
				scan((prev) => RNG.hash(prev), Constants.RandomSeed.valueOf()),
				map((unscaled) => RNG.scale(unscaled)),
				map(scaled => new RandomTeleport(scaled))
			)
	 ...
	 */
	const action$: Observable<Action> = merge(tick$, startLeftRotate$, startRightRotate$,
		stopLeftRotate$, stopRightRotate$, startThrust$, stopThrust$, teleport$, shoot$, randomTeleport$);
	const state$: Observable<State> = action$.pipe(scan(reduceState, initialState));
	const subscription: Subscription = state$.subscribe((state) => updateView(() => subscription.unsubscribe())(state))
