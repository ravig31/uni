...
export { initialState, reduceState, Rotate, Shoot, Tick, Thrust, UserTeleport, RandomTeleport}
...
class RandomTeleport implements Action {
	constructor(public readonly hash: number) { }

	apply = (s: State): State => ({ ...s,
		ship: { ...s.ship,
			pos : new Vec(Constants.CanvasSize / 2 + (this.hash * Constants.CanvasSize),
						Constants.CanvasSize / 2 + (this.hash * Constants.CanvasSize),)
		}
	})
}

class UserTeleport implements Action {
	
	apply = (s: State): State => ({ ...s,
		ship: {...s.ship,
			pos : s.ship.pos.add(Vec.unitVecInDirection(s.ship.angle)
						.scale(Constants.TeleportDistance))
		} 
	})
}
...
class Tick implements Action {
    ...
    static handleCollisions = (s: State): State => {
        const
			...
			newScore = s.score + collidedBulletsAndRocks.length;
        return {
            ...s,
